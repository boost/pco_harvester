...
export const updateTransformedRecord = createAsyncThunk(
  ...
  async (payload) => {
    ...
    const { contentPartnerId, transformationDefinitionId, fields, record } = payload;

    const response = axios.post(`/content_partners/${contentPartnerId}/transformation_definitions/${transformationDefinitionId}/fields/run`, {
      record: record,
      fields: fields,
    })
      .then(response => {
...
const AppDetailsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
    .addCase(updateTransformedRecord.fulfilled, (state, action) => {
