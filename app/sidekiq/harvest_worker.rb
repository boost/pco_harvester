...
class HarvestWorker
  ...
  def perform(harvest_job_id)
    ...
    @extraction_job = Job.new(extraction_definition: @harvest_definition.extraction_definition, kind: 'sample')
    ...
    @transformation = TransformationDefinition.new(@harvest_definition.transformation_definition.attributes.merge('job_id' => @extraction_job.id))
